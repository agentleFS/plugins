---
name: slack
description: Work across agentleFS (afs) and Slack together. Use when the user names Slack — asks to tell, ping or message someone in Slack about an afs document or folder ("tell Bob in Slack the doc needs a look"), asks what was said in Slack about something kept in afs, or wants a Slack thread's outcome saved into afs — or when Slack's own tools are connected in this session and the user asks for the latest on a topic that may be split between written docs and Slack conversation. Not for ordinary document questions when Slack is neither named nor connected; that is the documents skill.
---
...
user sees in Slack, nothing more. That is deliberate: Slack's API terms bar third-party
apps from keeping copies of Slack data, and a live read with the person's own permissions
is the only version that is both allowed and exactly right.
...
A missing Slack connection is not "nothing in Slack about this"; say which one it is.
...
1. **Find the doc.** `search` or `read_org_doc`, until you hold its exact
...
2. **Check whether that person can read it.** `who_can_read` on the doc's location with
   ...
   - **Nobody outside the organization is ever on it.** A Slack person who is not a
     member cannot open the doc through this organization. The only way across is a
     share offered to their own organization (`share` with `action: "share_out"`, in the
     `sharing` skill), which is a bigger decision than a Slack message and the user's
     to make.
...
3. **Act on the answer:**
   ...
   | You cannot tell — `not found`, a refusal, a group whose members you cannot list, a roster of bare ids with no names, or no confident name match | Say which, and ask whether to send anyway. Hold off offering a new grant on an answer you could not read: widening access to fix a problem that may not exist is the one mistake here that outlives the message. |
...
1. Search both, with the user's words: `search`, and Slack's search.
